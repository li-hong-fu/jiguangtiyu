<template>
  <div class="dialog">
    <el-dialog :title="state == 'edit' ? '编辑' : '新增'" custom-class="el-dialog-common" :visible.sync="visible">
      <el-form :ref="formRef" :rules="rules" :model="form" label-width="80px" size="small">
        <el-form-item label="菜单分类" prop="pid">
          <el-select v-model="form.pid" filterable clearable placeholder="请选择分类">
            ...
            <template v-for="item in tableList">
              <el-option :class="'option-level' + item.level" :label="item.level != 1 ? '└─ ' + item.title : item.title"
        ...
        <el-form-item label="菜单名称" prop="name">
          <el-col :span="23">
            <el-input clearable v-model="form.name" auto-complete="off" placeholder="请输入菜单名称"></el-input>
          </el-col>
        </el-form-item>
        ...
        <el-form-item label="权限URL" prop="url" v-show="form.type == '' || form.type == 3 || form.type == 4">
          <el-col :span="23">
            <el-input clearable v-model="form.url" auto-complete="off"></el-input>
          </el-col>
        </el-form-item>

        <el-form-item label="路由URL" prop="router" v-show="form.type == '' || form.type == 2 || form.type == 1">
          <el-col :span="23">
            <el-input clearable v-model="form.router" auto-complete="off"></el-input>
          </el-col>
        </el-form-item>
        ...
        <el-form-item label="是否显示" prop="display">
          <el-col :span="23">
            <el-radio clearable v-model="form.display" v-for="item in displayRadio" :key="item.id" :label="item.id">
              {{ item.name }}</el-radio>
          </el-col>
        </el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //获取详情
      getDetail() {
        modeleMenu.detail(this.id).then((res) => {
          if (res.code == 200) {
            this.form = {
              id: res.data.id,
              pid: res.data.pid,
      ...
      //编辑子类
      onEdit(params) {
        modeleMenu.update(this.id, params).then((res) => {
          if (res.code == 200) {
            this.$msg.success(res.data);
            this.loading = false;
            this.$parent.getList();
